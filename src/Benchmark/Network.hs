...
import Benchmark.Types (Endpoint (..), Nanoseconds (..), PerfTestError (..), Settings (..), TestingResponse (..))
...
timedRequestPrepared :: Manager -> Request -> IO TestingResponse
timedRequestPrepared mgr req = retry 3 1_000_000
  where
    retry :: Int -> Int -> IO TestingResponse
    retry attempts delay = do
        ...
        case result of
            ...
            Left err | attempts > 0 -> do
                ts <- getTimestamp
                putStrLn $ "\n" ++ ts ++ " [!] Connection Failed: " ++ show err ++ "\n    Retrying (" ++ show attempts ++ " left)..."
                threadDelay delay
                retry (attempts - 1) (delay * 2)
            Left err -> do
                ts <- getTimestamp
                endTime <- getTime Monotonic
                putStrLn $ "\n" ++ ts ++ " [!] FINAL FAILURE: " ++ show err
                return $
                    TestingResponse
...
runBenchmark settings sem mgr iters pIdx endpoint = do
    ...
    replicateConcurrently iters $ do
        bracket_ (waitQSem sem) (signalQSem sem) $ do
            res <- timedRequestPrepared mgr preparedReq

            current <- atomicModifyIORef' countRef (\n -> (n + 1, n + 1))
            printProgressBar pIdx current iters

            return res
  where
    printProgressBar idx c total = do
        ...
        when shouldPrint $ do
            ...
            let msg = "[Ep " ++ show idx ++ "] Progress: " ++ show percent ++ "% (" ++ show c ++ "/" ++ show total ++ ")"
            putStrLn msg
...
runComparison settings mgr endpointA endpointB = do
    ...
    concurrently (timedRequestPrepared mgr reqA) (timedRequestPrepared mgr reqB)
