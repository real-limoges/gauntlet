...
import Benchmark.Types (BenchmarkOutput (..), BenchmarkStats (..), Endpoint (..), MetricRegression (..), Nanoseconds (..), OutputConfig (..), OutputFormat (..), PayloadResult (..), PerfTestError (..), RegressionResult (..), RunResult (..), Settings (..), Targets (..), TempoSettings, TestConfig (..), TestingResponse (..), TraceOutput (..), defaultThresholds, exitWithError)
...
logMsg :: OutputFormat -> String -> IO ()
logMsg OutputTerminal msg = putStrLn msg
logMsg OutputJSON _ = return ()
...
initContext setts csvFile timestamp eventChan = do
    ...
    return RunContext{rcSettings = setts, rcManager = mgr, rcToken = token, rcCsvFile = csvFile, rcTimestamp = timestamp, rcEventChan = eventChan}
...
runMultipleWithTUI baselineMode cfg csvFile timestamp epsCandidate epsPrimary setts = do
    ...
    runTraceAnalysis OutputTerminal (rcManager ctx) setts timestamp startNs endNs

    handleBaseline OutputTerminal baselineMode (T.pack timestamp) statsCandidate
...
runMultipleJSON baselineMode cfg csvFile timestamp epsCandidate epsPrimary setts = do
    ...
    logMsg OutputJSON "Starting with Candidate ..."
    setupOrFail (candidate $ git cfg) (candidate $ targets cfg)
    resultsCandidate <- benchmarkEndpoints OutputJSON ctx "candidate" epsCandidate

    logMsg OutputJSON "Starting with Primary"
    setupOrFail (primary $ git cfg) (primary $ targets cfg)
    ...
    handleBaseline OutputJSON baselineMode (T.pack timestamp) statsCandidate
...
runSingleWithTUI baselineMode cfg csvFile timestamp eps setts targetUrl = do
    ...
    handleBaseline OutputTerminal baselineMode (T.pack timestamp) stats
...
runSingleJSON baselineMode cfg csvFile timestamp eps setts targetUrl = do
    ...
    handleBaseline OutputJSON baselineMode (T.pack timestamp) stats
...
runEndpointLoop outFmt RunContext{..} endpoints = do
    ...
    logMsg outFmt $ "Starting " ++ show (length endpoints) ++ " endpoints with " ++ show conc ++ " concurrency in parallel..."
...
runWarmup outFmt RunContext{..} ep = do
    logMsg outFmt "Warming up..."
    sem <- newQSem 1
    let authorizedEp = addAuth rcToken ep
    _ <- runBenchmark rcSettings sem rcManager 1 1 authorizedEp
    return ()
...
runTraceAnalysis :: OutputFormat -> Manager -> Settings -> String -> TT.Nanoseconds -> TT.Nanoseconds -> IO ()
runTraceAnalysis outFmt mgr setts timestamp startNs endNs = case tempo setts of
    Nothing -> return ()
    Just tempoSetts
        ...
        | otherwise -> do
            ...
            case result of
                Left err -> logWarning $ "Failed to fetch traces: " ++ show err
                Right (Left err) -> logWarning $ "Tempo error: " ++ err
                Right (Right []) -> logWarning "No traces found for time range"
                Right (Right traces) -> do
                    let tracesFile = resultsDir ++ "/traces-" ++ timestamp ++ ".json"
                    writeRawTraces tracesFile traces
                    logInfo $ "Raw traces written to: " ++ tracesFile
...
handleBaseline :: OutputFormat -> BaselineMode -> Text -> BenchmarkStats -> IO RunResult
handleBaseline outFmt mode timestamp stats = case mode of
    NoBaseline -> return RunSuccess
    SaveBaseline name -> do
        result <- saveBaseline name timestamp stats
        case result of
            Left err -> do
                logMsg outFmt $ "Error: " ++ err
                return RunSuccess -- Save failure is not a regression
            Right path -> do
                logMsg outFmt $ "Baseline saved: " ++ path
                return RunSuccess
    ...
    SaveAndCompare saveName compareName -> do
        ...
        case saveResult of
            Left err -> logMsg outFmt $ "Warning: Failed to save baseline: " ++ err
            Right path -> logMsg outFmt $ "Baseline saved: " ++ path
