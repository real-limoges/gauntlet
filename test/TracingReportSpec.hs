...
import TastyCompat (shouldSatisfy)
...
tracingReportSpec =
  sequentialTestGroup
    ...
    [ let agg =
       ...
       in testGroup
            "formatSpanRow"
            [ testCase "contains span name" $
                row `shouldSatisfy` ("GET /api/users" `isInfixOf`)
            , testCase "contains count" $
                row `shouldSatisfy` ("42" `isInfixOf`)
            , testCase "formats decimal values with 2 decimal places" $
                row `shouldSatisfy` ("12.34" `isInfixOf`)
            , testCase "truncates span names longer than 50 characters" $ do
    ...
    , testGroup
        "printSpanTable"
        [ testCase "prints 'No spans to display.' for empty list" $ do
            output <- captureStdout (printSpanTable [])
            output `shouldSatisfy` ("No spans to display." `isInfixOf`)
        , testCase "prints header for non-empty list" $ do
            let agg =
                  SpanAggregation
                    { aggSpanName = "test"
                    , aggCount = 1
