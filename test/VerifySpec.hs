...
verifySpec = describe "Benchmark.Verify" $ do
  ...
  describe "verify with ignoreFields" $ do
    it "ignores specified top-level key in comparison" $ do
    ...
    it "still detects mismatch on non-ignored key" $ do
      let body1 = "{\"id\":\"abc\",\"value\":1}"
    ...
    it "ignores specified nested key at any depth" $ do
  ...
  describe "verifyWithNetworkCheck" $ do
    it "returns NetworkError when primary has errorMessage" $ do
    ...
    it "returns NetworkError when candidate has errorMessage" $ do
      let r1 = makeResponseWithBody 200 "{}"
    ...
    it "delegates to verify when neither has errorMessage" $ do
      let body = "{\"ok\":true}"
      let r1 = makeResponseWithBody 200 body
      let r2 = makeResponseWithBody 200 body
