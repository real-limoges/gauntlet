...
configSpec =
  testGroup
    ...
    , testGroup
        ...
        , testCase "accepts valid retry configuration" $ do
            ...
            case retry (settings cfg) of
              ...
              Nothing -> assertFailure "Expected retry settings"
        , testCase "accepts retry with zero attempts (disable retry)" $ do
            ...
            case retry (settings cfg) of
              ...
              Nothing -> assertFailure "Expected retry settings"
        , testCase "accepts various backoff multipliers" $ do
    ...
    , testGroup
        "warmup settings"
        [ testCase "uses default warmup settings when not specified" $ do
            let cfg = makeValidConfig
            warmup (settings cfg) `shouldBe` Nothing
        , testCase "accepts valid warmup configuration" $ do
            ...
            case warmup (settings cfg) of
              ...
              Nothing -> assertFailure "Expected warmup settings"
        , testCase "accepts warmup with zero iterations (disabled)" $ do
            ...
            case warmup (settings cfg) of
              ...
              Nothing -> assertFailure "Expected warmup settings"
        , testCase "accepts large warmup iteration counts" $ do
